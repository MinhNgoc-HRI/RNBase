...
import { useEvent, useVectorLayout } from '@src/components/Tooltip/hook';
import { getCurrentNode, withAnimation } from '@src/components/Tooltip/math';
import MenuButton from '@src/components/Tooltip/components/MenuButton';
import type { SceneType } from '@src/components/Tooltip/types';
...
function MashView(props: MashViewProps) {
  ...
  const { nodes, options } = useContext(TooltipContext);
  ...
  const animatedPropsBackdrop = useAnimatedProps(() => {
    return {
      fill: interpolateColor(
        ...
        [
          'rgba(0, 0, 0, 0)',
          `rgba(0, 0, 0, ${options?.backdropOpacity ?? 0.8})`,
        ],
      ),
    };
  });
  ...
  const animatedPropsMaskCircle = useAnimatedProps(() => {
    ...
    return {
      ...
      r:
        maskType === 'circle'
          ? withAnimation(
              Math.max(target.width.value, target.height.value) / 2,
              pathAnimated,
            )
          : 0,
    };
  ...
  const animatedPropsMaskRect = useAnimatedProps(() => {
    ...
    return {
      ...
      width:
        maskType !== 'circle'
          ? withAnimation(target.width.value, pathAnimated)
          : 0,
      height:
        maskType !== 'circle'
          ? withAnimation(target.height.value, pathAnimated)
          : 0,
    };
  });
  ...
  const childrenStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      backgroundColor: interpolateColor(
        ...
        [
          'transparent',
          options?.colorNodeOnPress ?? 'rgba(255, 255, 255, 0.8)',
        ],
      ),
      transform: [
  ...
  const contentStyle = useAnimatedStyle(() => {
    const { node } = getCurrentNode(nodes, scene, currentStep, defaultTarget);
    const { target } = node;
    const isTargetTopScreen =
      target.y.value + target.height.value < dimension.height / 2;
    const translateY = isTargetTopScreen
      ? withAnimation(target.y.value + target.height.value + TriangleHeight)
      : withAnimation(target.y.value - contentHeight.value - TriangleHeight);

    const isOverScreen =
      contentWidth.value + target.x.value > dimension.width - 32;
    const delta = (dimension.width - contentWidth.value) / 2;
    const translateX = isOverScreen
      ? withAnimation(Math.max(0, delta))
      : withAnimation(target.x.value);
  ...
  const triangleStyle = useAnimatedStyle(() => {
    ...
    const translateX =
      maskType === 'circle'
        ...
        : withAnimation(target.x.value - originTranslateX + 12);
  ...
  const stepStyle = useAnimatedStyle(() => {
    ...
    const translateX =
      target.x.value < 32
        ? target.x.value + target.width.value - StepHeight / 2
        : target.x.value - StepHeight / 2;
      ...
      {/* {options?.stepShow !== false && (
        <Animated.View
          ...
          <AnimateableText
            style={[
              styles.txtStep,
              { color: options?.stepTitleColor ?? 'white' },
            ]}
