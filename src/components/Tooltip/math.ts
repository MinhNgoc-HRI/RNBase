...
export function withAnimation(value: number, animated = true) {
  'worklet';
...
// a r,r 0 1,1 -(r * 2),0
// "
export function circlePathCalculator(cx: number, cy: number, r: number) {
  'worklet';
...
export function circlePath(target: TargetType) {
  'worklet';
...
export function rectPath(target: TargetType) {
  'worklet';

  const pathRects = [
    `M${target.x.value} ${target.y.value}`,
    `L${target.x.value} ${target.y.value + target.height.value}`,
    `L${target.x.value + target.width.value} ${
      target.y.value + target.height.value
    }`,
...
export function getRectPath(target: TargetType) {
  'worklet';
