...
export function useTooltip() {
  const { nodes, setSceneIndex, options } = React.useContext(TooltipContext);
...
export function useEvent() {
  const addEventListener = useCallback(
    (eventName: 'TooltipEvent', callback: (data: EventData) => void) => {
  ...
  const emitEvent = useCallback(
    (eventName: 'TooltipEvent', data: EventData) => {
...
export const useVectorLayout = (x = 0, y = 0, width = 0, height = 0) => {
...
export const useVector = (x = 0, y = 0) => {
  const targetX = useSharedValue(x);
  const targetY = useSharedValue(y);
...
export const setVector = (target: Vector, { x = 0, y = 0 }) => {
  'worklet';
